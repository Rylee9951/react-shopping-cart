...
const Cart = props => {
  ...
  useEffect(() => {
    const amount = newCart.reduce((a, b) => {
      return a + b.finalCart.price * b.quantity
    }, 0)
          ...
          <div key={item.finalCart.id} className="cartItem">
            <div className="cartLeft">
              <div>
                <img
                  className="cartImage"
                  src={`/assets/${item.finalCart.sku}_2.jpg`}
                  alt={item.finalCart.title}
                />
              </div>
            ...
            <div>
              <div className="delete" onClick={e => remove(item.finalCart.id)}>
                <i className="fa fa-trash"></i>
              </div>
              <p className="cartPrice">
                ${(item.quantity * item.finalCart.price).toFixed(2)}
              </p>
            </div>
          </div>
        ))}
