          ...
          <div id="itemsContainer">
            {items.map(item => (
              <div key={item.id} className="itemContainer">
                <div
                  className={item.isFreeShipping ? `shipping` : "noShipping"}
                >
                ...
                <p className="installments">{`or ${item.installments} x${
                  item.currencyFormat
                }${(item.price / item.installments).toFixed(2)}`}</p>
                <button onClick={e => add(item.id)} className="addButton">
                  Add to cart
                </button>
              </div>
            ))}
